...
addEventListener("message", function(event){
    ...
    if(item.open == true) {
        ...
        $.post('https://qb-bossmenu/closeNUI');
    }
});
...
$(document).on('click', '.fireemployee', function(e) {
    ...
    $.post('https://qb-bossmenu/fireEmployee', JSON.stringify({
        source: playerid,
    }));
});
...
$(document).on('click', '.grade-box', function(e) {
    ...
    $.post('https://qb-bossmenu/changeGrade', JSON.stringify({
        grade: gradeid,
...
$(document).on('click', '#option-recruit', function(e) {
    ...
    $.post('https://qb-bossmenu/openRecruit', JSON.stringify({}));
});

$(document).on('click', '#option-bosstash', function(e) {

    $.post('https://qb-bossmenu/openStash', JSON.stringify({}));
});

$(document).on('click', '#option-outfit', function(e) {

    $.post('https://qb-bossmenu/outfit', JSON.stringify({}));
});
...
$(document).on('click', '.givejob', function(e) {
    ...
    $.post('https://qb-bossmenu/giveJob', JSON.stringify({
        source: playerid,
    }));
});
...
$(document).on('click', '.deposit', function(e) {
    $.post('https://qb-bossmenu/deposit', JSON.stringify({
        amount: currentDepositAmount,
    }));
});

$(document).on('click', '.withdraw', function(e) {
    $.post('https://qb-bossmenu/withdraw', JSON.stringify({
